...
제공된 강의 텍스트나 대본을 바탕으로 핵심 내용을 깔끔하고 임팩트 있게 제목처럼 요약해야 합니다.
문장 형태가 아닌 제목 스타일로 핵심 키워드와 개념을 간결하게 나열하는 것이 중요합니다."""

SUMMARIZE_LECTURE_USER_TEMPLATE = """다음 강의 내용을 제목처럼 깔끔하고 임팩트 있게 요약해주세요:

{lecture_content}

다음 형식으로 요약해주세요:
- 문장 형태가 아닌 제목 스타일로 작성
- 핵심 키워드와 개념을 간결하게 나열
- 불필요한 설명이나 접속사 없이 핵심만 표현
- 각 항목은 독립적인 제목처럼 작성

예시:
❌ "오늘 강의에서는 파이썬의 리스트와 튜플에 대해 배웠고, 리스트는 가변 자료구조이며 튜플은 불변 자료구조라는 차이점을 학습했습니다."
✅ "파이썬 자료구조: 리스트(가변) vs 튜플(불변)"
