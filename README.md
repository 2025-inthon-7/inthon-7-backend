...
본 시스템은 웹소켓을 통해 실시간으로 학생들의 이해도 피드백, 질문 등을 교수에게 전달하며, Google Generative AI를 연동하여 학생 질문에 대한 1차 답변을 생성하고, 질문 내용을 명료하게 다듬는 기능을 제공합니다.
...
- **대화형 Q&A 시스템**:
  ...
  - 등록된 질문은 AI가 먼저 분석하여 내용을 명료하게 다듬고 (Text Cleaning), 관련 답변을 생성합니다.
  - 다른 학생들은 "나도 궁금해요" 기능을 통해 질문에 공감할 수 있습니다.
  - AI 답변이 부족할 경우, 교수는 직접 답변하거나 질문을 추가적으로 처리할 수 있습니다.
...
InThon은 다음과 같은 구성 요소들이 유기적으로 상호작용하여 동작합니다.
...
2.  **Django Channels (WebSocket Server)**:
    - `Daphne` ASGI 서버 위에서 동작하며, 교수와 학생 클라이언트 간의 실시간 양방향 통신을 담당합니다.
    - `Redis`를 Channel Layer 백엔드로 사용하여 여러 서버 인스턴스 간의 메시지 브로드캐스팅을 지원합니다.

3.  **Celery (Task Queue)**:
    - AI 모델 호출과 같이 시간이 오래 걸리는 작업을 비동기적으로 처리합니다.
